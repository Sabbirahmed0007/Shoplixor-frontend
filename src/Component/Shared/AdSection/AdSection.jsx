...
const AdSection = ({image, heading, description, address}) => {
    return (
        <div className='bg-stone-200'>
            <div className=' w-10/12 mx-auto py-10 gap-8 grid grid-cols-1 lg:grid-cols-2 items-center justify-evenly'>
                ...
                <div className=' lg:mx-10 font-playfair'>
                    <h1 className='font-extrabold text-3xl'>{heading}</h1>
                    <p className='my-4 text-justify font-medium'>{description}</p>
                    <Link to={address} className='btn font-semibold border-none drop-shadow-xl rounded-full text-white bg-green-600 hover:bg-green-700'> Shop Now</Link>
                </div>
            </div>
        </div>
    );
